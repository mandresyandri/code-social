...
def analyze_and_code_segment(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.warning(
                f"Erreur lors de l'analyse du segment (tentative {attempt+1}): {e}"
            )
            if attempt == max_retries:
    ...
    logger.warning(
        "Impossible d'obtenir un JSON valide ou avec extraits longs, retour d'une liste vide"
    )
    return []
            ...
            for sentence in sentences:
                if short_excerpt in sentence and len(sentence.split()) >= 10:
                    fallback_codes.append(
                        {"code": code_text, "excerpt": sentence.strip()}
                    )
                    break
...
def cluster_codes_limited(
    ...
    try:
        ...
        logger.info(
            f"Clustering terminé: {len(clusters)} thèmes identifiés (limité à {max_clusters})"
        )
...
def meta_cluster_themes(
    ...
    for cluster_id, codes in clusters.items():
        ...
        codes_text = "; ".join(
            [f"{c['code']}: {c['excerpt']}" for c in codes[:3]]
        )  # Limiter pour éviter trop de tokens
        theme_descriptions[cluster_id] = (
            f"Thème '{theme_name}' avec codes: {codes_text}"
        )
    ...
    try:
        ...
        if start != -1 and end != -1:
            ...
            try:
                ...
                meta_themes = {
                    int(k): v for k, v in data.get("meta_themes", {}).items()
                }
                assignments = {
                ...
                for theme_id in clusters.keys():
                    if theme_id not in assignments:
                        ...
                        if meta_themes:
                            ...
                            meta_counts = {
                                mt: sum(1 for t, m in assignments.items() if m == mt)
...
def compile_results_with_meta(
    ...
    for code_dict in all_codes:
        ...
        rows.append(
            {"Méta-thème": meta_theme, "Thème": theme, "Code": code, "Extrait": excerpt}
        )
...
def run_pipeline(
    pdf_path: str, output_path: str = "resultats_analyse.csv", max_themes: int = 10
):
    """
    ...
    try:
        ...
        model_cot = (
            "meta-llama/llama-4-maverick-17b-128e-instruct"  # Pour l'analyse CoT
        )
        ...
        if len(clusters) > max_themes:
            logger.info(
                f"Second regroupement pour atteindre {max_themes} thèmes maximum"
            )
        ...
        meta_summary = (
            df.groupby("Méta-thème")
            .count()[["Code"]]
            .rename(columns={"Code": "Nombre de codes"})
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Pipeline d'analyse qualitative sociologie du numérique"
    )
    parser.add_argument("pdf_path", help="Chemin vers le PDF d'entretien")
    parser.add_argument(
        "--output",
        default="resultats_analyse.csv",
        help="Chemin pour enregistrer les résultats",
    )
    parser.add_argument(
        "--max-themes", type=int, default=10, help="Nombre maximum de thèmes à générer"
    )
